...
class EvilWizard:
    ...
    def pathfind(self):
        ...
        if(self.map[int((self.pos[1]+32)//64)][int((self.pos[0]+32)//64)] != 0 or self.map[int((self.player.pos[1]+32)//64)][int((self.player.pos[0]+32)//64)]):
            return
    ...
    def move(self,dt):
        ...
        if(self.target[0] == -1 or self.animtime % 1 == 0):
            self.pathfind()
        # print(self.target)
